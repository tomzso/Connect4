...
play(Matrix) ->
  {Cols, Rows, Connect} = game_config(),
  WinnerResult = check_winner(Cols, Rows, Connect, Matrix),
  case WinnerResult of
    none ->
      ok;  % do nothing if no winner
    Winner ->
...
get_Diagonal_left(_, _, 0  ,_) -> []; % from top left to botton right (in matrix)
...
get_Diagonal_right(_, _, 0 , _) -> []; % from top right to botton left (in matrix)
...
ai_move(Matrix) ->
  ...
  Max_value = max_list(Heuristics_score_list),
  io:format("List: ~p~n", [Heuristics_score_list]),

  Ai_move_col_index = find_index_by_value(Max_value, Heuristics_score_list),
...
all_move_score(ColIndex, RowIndex, ColDiagonalEdge, RowDiagonalEdge, Connect, Matrix , Player) ->
  [
    case move(Matrix, ColIndex, [], Player, notMoved) of
      ...
      {moved, NewMatrix } ->
        {C, _, _} = game_config(),
       calc_move_score(C, RowIndex, ColDiagonalEdge, RowDiagonalEdge, Connect, NewMatrix , Player)
    end
